...
modify_date_str = "Modify Date"
modify_date_flag = False

create_date_str = "Create Date"
    ...
    for tag in parsed_tag_arr[0]:
        ...
        for attr in data.split('\n'): 
            ...
            if str(tag).rstrip() in str(attr).rstrip():
                if(modify_date_flag and tag == modify_date_str):
                    continue
                if(create_date_flag and tag == create_date_str):
                    continue

                if(tag == modify_date_str):
                    modify_date_flag = True
                if(tag == create_date_str):
